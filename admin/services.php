...
if (strlen($_SESSION['salondbaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (move_uploaded_file($_FILES["serviceImage"]["tmp_name"], $targetFile)) {
            // Insert the service with the image path into the database
            $query = mysqli_query($con, "INSERT INTO tblservices (ServiceName, CategoryName, Cost, Duration, ServiceDescription, MaxAppointmentsPerDay, ImagePath) VALUES ('$sername', '$category', '$cost', '$duration', '$description', '$maxAppointments', '$targetFile')");
    ...
    if (isset($_POST['updateService'])) {
        ...
        $description = $_POST['description'];
        $maxAppointments = $_POST['maxAppointments'];
        $promotion = $_POST['promotion']; // Get the promotion value
        $discountPrice = $_POST['discountPrice']; // Get the Discount Price value

        // Prepare the update statement
        $stmt = $con->prepare("UPDATE tblservices SET ServiceName=?, CategoryName=?, Cost=?, Duration=?, ServiceDescription=?, MaxAppointmentsPerDay=?, promo=?, DiscountPrice=? WHERE ID=?");
        $stmt->bind_param("ssisssssi", $sername, $category, $cost, $duration, $description, $maxAppointments, $promotion, $discountPrice, $serid);

        // Check if a new image has been uploaded
        if (!empty($_FILES["serviceImage"]["name"])) {
            // Handle the new image upload
            $targetDir = "../uploads/";
            $imageFileType = strtolower(pathinfo($_FILES["serviceImage"]["name"], PATHINFO_EXTENSION));
            $targetFile = $targetDir . uniqid() . '.' . $imageFileType; // Use a unique name

            // Validate the image
            $check = getimagesize($_FILES["serviceImage"]["tmp_name"]);
            if ($check === false || $_FILES["serviceImage"]["size"] > 2000000 || !in_array($imageFileType, ["jpg", "png", "jpeg"])) {
            ...
            if (move_uploaded_file($_FILES["serviceImage"]["tmp_name"], $targetFile)) {
                ...
                $stmt = $con->prepare("UPDATE tblservices SET ServiceName=?, CategoryName=?, Cost=?, Duration=?, ServiceDescription=?, MaxAppointmentsPerDay=?, ImagePath=?, DiscountPrice=? WHERE ID=?");
                $stmt->bind_param("ssisssssi", $sername, $category, $cost, $duration, $description, $maxAppointments, $targetFile, $discountPrice, $serid);
            } else {
...
<div class="container-scroller">
                                ...
                                <div class="modal-body">
                                    <form method="POST" action="services.php" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <img id="currentImage" src="" alt="Current Service Image" style="width: 100px; height: auto;">
                                        </div>
                                        <div class="form-group">
                                            <label for="editServiceImage">Upload New Image (optional)</label>
                                        ...
                                        <div class="form-group">
                                            <label for="editDiscountPrice">Discount Price (optional)</label>
                                            <input type="number" class="form-control" id="editDiscountPrice" name="discountPrice">
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="Promotion" name="promotion" required>
                                                <option value="yes" <?php echo ($row['promo'] == 'yes') ? 'selected' : ''; ?>>Yes</option>
                                                <option value="none" <?php echo ($row['promo'] == 'none') ? 'selected' : ''; ?>>None</option>
                                            </select>
                                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if (!empty($row['ImagePath'])): ?>
                                                    <img src="<?php echo htmlspecialchars($row['ImagePath']); ?>" alt="<?php echo htmlspecialchars($row['ServiceName']); ?>" style="width: 50px; height: auto;">
                                                <?php else: ?>
                                                    No image
                                                <?php endif; ?>
                                            </td>
                                            ...
                                            <td>
                                                <?php
                                                // Check if the service has a discounted price
                                                if (!empty($row['DiscountPrice'])) {
                                                    echo '₱   ' . htmlspecialchars($row['DiscountPrice']);
                                                } else {
                                                    echo 'N/A';  // Display 'N/A' if there is no discount
                                                }
                                                ?>
                                            </td>
                                            ...
                                            <td>
                                                <button class="btn btn-warning btn-sm" 
                                                    onclick="editService(
                                                        ...
                                                        '<?php echo htmlspecialchars($row['ImagePath']); ?>',
                                                        '<?php echo $row['Promo']; ?>',
                                                        '<?php echo $row['DiscountPrice']; ?>' // Pass the DiscountPrice value
                                                    )">
                                                    Edit
                                                </button>
    ...
    <script>
        ...
        function editService(id, name, cost, category, maxAppointments, description, duration, imagePath, promotion, discountPrice) {
            ...
            $('#currentImage').attr('src', imagePath); // Set the current image source
            $('#Promotion').val(promotion); // Set the promotion value in the dropdown
            $('#editDiscountPrice').val(discountPrice); // Set the discount price field value
